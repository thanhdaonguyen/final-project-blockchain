...
function printHelp() {
  ...
  if [ "$USAGE" == "prereq" ]; then
    ...
    println "    -ccep <policy>  - (Optional) Chaincode endorsement policy using signature policy syntax. The default policy requires an endorsement from UET and HUS"
    ...
    println
    println " Possible Mode and flag combinations"
    println "   \033[0;32mdeployCCAAS\033[0m -ccn -ccv -ccs -ccp -cci -r -d -verbose"
    println
    println " Examples:"
    ...
    println
    println "    Flags:"
    println "    -org <name>       - Org name for executing the command (UET, HUS, etc) (default is UET)."    
    ...
    println
    println " Examples:"
    println "   network.sh cc list -org UET"
